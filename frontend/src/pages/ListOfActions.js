...
import { getScopedInAsset } from '../api/scopeAPI';
import {
...
const ListOfActions = () => {
  ...
  const handleMarkAsCompleted = async (actionId) => {
    try {
      const response = await axios.put(`http://localhost:8021/api/v1/actions/${actionId}`, { isCompleted: true, assetId: selectedAsset, scopeId: selectedScope });
  ...
  const handleAssetChange = async (event) => {
    ...
    try {
      const scopesData = await getScopedInAsset(assetId);
